...
Login exists solely to establish authenticated sessions. Every design and UX decision should optimise for the pipe flow: copy URL → open Piper → content lands in Instapaper. A clunky pipe means Piper has failed, regardless of how polished the login screen is.
...
## 6. Fail Loudly
Never silently fail or retry in the background. If extraction fails, cookies are stale, or the worker is down — tell the user immediately. They need to know to try again. The UI must always reflect the true outcome.

## 7. Content Fidelity Over Speed
Better to take 3 extra seconds and extract cleanly than to deliver broken HTML that Instapaper cannot parse. The user's patience is highest at the pipe moment. Spend it on correctness.
